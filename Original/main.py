...
def build_system(reviewer_pool):
    seed = [
        {"reviewer_id": r.reviewer_id, "name": r.name,
         "conflicts": r.conflicts, "workload": r.workload}
        for r in reviewer_pool
    ]
    db = Database(reviewers=seed)
    researcher = Researcher("Dr Khumalo")
    notifier = NotificationService(researcher)
    evaluator = EvaluationManager(db, notifier)
    rm = ReviewerManager(db, reviewer_pool)
    controller = SubmissionController(Validator(), db, rm, evaluator)
    ui = UI(controller)
